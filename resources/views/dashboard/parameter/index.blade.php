...
<div class="alert alert-warning alert-dismissible fade show" role="alert">
    Tidak ada Parameter yang dapat di tampilkan <strong><a href="{{ route('parameter.create') }}"> Buat Parameter Sekarang</a></strong>
    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
</div>

@else
<a href="{{ route('parameter.create') }}" class="btn bg-info text-white mb-5"><i class="ti ti-circle-plus"></i> Tambah parameter</a>
        ...
        <tbody>
            @foreach($parameters as $parameter)
                <tr>
                    ...
                    <td>{{ $parameter->categoryParameter->name }}</td> <!-- Menampilkan nama kategori -->
                    <td>{{ $parameter->metode->name }}</td> <!-- Menampilkan nama metode -->
                    <td>{{ number_format($parameter->harga, 0, ',', '.') }}</td>
                    <td class="align-middle">
                        {{-- <a href="{{ route('parameter.show', $parameter->id) }}" class="badge bg-info"><i class="ti ti-eye-check"></i></a> --}}
                        <a href="{{ route('parameter.edit', $parameter->id) }}" class="badge bg-warning"><i class="ti ti-edit"></i></a>
