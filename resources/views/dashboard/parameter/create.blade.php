...
<div class="d-flex align-items-center justify-content-center" style="height: 100vh;">
    <div class="card col-md-6">
        <div class="card-body">
            <h1 class="text-center">Tambah Parameter</h1>
            <form action="{{ route('parameter.store') }}" method="POST">
                @csrf

                <div class="mt-3 form-floating">
                    <input type="text" value="{{ old('name') }}" class="form-control @error('name') is-invalid @enderror " id="parameter" name="name" placeholder="Parameter">
                ...
                <div class="mt-3 form-group">
                    <select class="form-select @error('metode_id') is-invalid @enderror" name="metode_id">
                        ...
                        @if (old('metode_id') == $metode->id)
                        <option value="{{ $metode->id }}" selected>{{ $metode->name }}</option>
                        @else
                ...
                <div class="mt-3 form-floating">
                    <input step="any" type="text" class="form-control @error('harga') is-invalid @enderror" id="harga" name="harga" placeholder="Harga" value="{{ old('harga') }}">
